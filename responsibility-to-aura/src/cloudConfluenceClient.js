...
class CloudConfluenceClient {
  ...
  makeRequest(method, path, body = null, retryState = null, opts = {}) {
    ...
    // opts.noBasePath: when true, do NOT prepend this.basePath (/wiki).
    // Used for Jira endpoints on the same tenant host (/rest/api/3/...)
    // where the same email:token auth works but the /wiki prefix doesn't.
    const effectivePath = (opts.noBasePath ? "" : this.basePath) + path;
    ...
    return new Promise((resolve, reject) => {
      ...
      const retry = (newState) =>
        this.makeRequest(method, path, body, newState, opts)
          .then(resolve)
          .catch(reject);
  ...
  async updatePageStorage(pageId, title, type, storageXml, currentVersion, versionMessage) {
    const payload = {
      ...
      version: {
        ...
        message: versionMessage || "Storage rewrite",
      },
    };
    // Helper: return the server-truth new version number, falling back
    // to the payload's version if the API response didn't include it.
    const newVersionFrom = (response) =>
      (response && response.version && typeof response.version.number === "number")
    ...
    try {
      ...
      if (error.statusCode === 409) {
        try {
          ...
          return { success: false, error: `Version conflict retry failed: ${retryError.message}`, newVersion: null };
        }
      }
      return { success: false, error: error.message, newVersion: null };
    }
  }
  ...
  /**
   * Restore a prior page version using Confluence's native version
   * restore endpoint. Creates a new version that's a copy of the
   * specified historical version. This is the recommended rollback
   * mechanism — atomic, audited, and avoids byte-equality concerns.
   *
   *   POST /wiki/rest/api/content/{id}/version
   *   { "operationKey": "RESTORE", "params": { "versionNumber": N, "message": "..." } }
   *
   * @param {string|number} pageId
   * @param {number} prevVersionNumber - the historical version to roll forward
   * @param {string} message - audit message attached to the restore
  ...
  async restoreVersion(pageId, prevVersionNumber, message) {
    const payload = {
      ...
      params: {
        ...
        message: message || `Restore version ${prevVersionNumber}`,
      },
    };
    try {
  ...
  /**
   * Search Jira users by query string. Returns an array of user objects
   * with { accountId, displayName, emailAddress, active, ... }.
   *
   * Endpoint: GET /rest/api/3/user/search?query=<q>&maxResults=<n>
   * Docs: https://developer.atlassian.com/cloud/jira/platform/rest/v3/api-group-user-search/
   *
   * This is the only post-GDPR mechanism for resolving a free-text token
   * (email, displayName, or legacy DC username) to a Cloud accountId.
   * The Atlassian API token + email used for Confluence works for Jira
   * on the same tenant.
   *
   * Requires the calling user to have "Browse users and groups" global
   * permission in Jira; otherwise the endpoint returns 403.
   */
  async searchJiraUsersByQuery(query, limit = 50) {
    const encoded = encodeURIComponent(String(query));
    try {
      const response = await this.makeRequest(
        "GET",
        `/rest/api/3/user/search?query=${encoded}&maxResults=${limit}`,
        null,
        null,
        { noBasePath: true },
      );
