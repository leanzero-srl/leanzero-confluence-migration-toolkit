 ...
 *
 * Roll back a sync_responsibility_to_aura.js run.
 *
 * Two strategies:
 ...
 *   node main/restore_responsibility_to_aura.js --plan-file logs/plan_<id>.json --from-backup
 */
...
function parseArgs(argv) {
  const o = {
    ...
    fromBackup: false,
    concurrency: 3,
    message: "Revert Responsibility -> Aura migration",
    help: false,
  };
...
function help() {
  console.log(`
Usage: node main/restore_responsibility_to_aura.js --plan-file <path> [options]

  --plan-file PATH        plan JSON written by sync_responsibility_to_aura.js
...
class ResponsibilityToAuraRestore {
  constructor(opts) {
    this.opts = opts;
    ...
    fs.writeFileSync(
      this.logFile,
      `Responsibility -> Aura Restore Log\nStarted: ${new Date().toISOString()}\n${"=".repeat(80)}\n\n`,
    );
...
async function main() {
  ...
  try {
    const r = new ResponsibilityToAuraRestore(opts);
    await r.run();
    console.log("\nDone.");
    process.exit(0);
  } catch (e) {
...
module.exports = { ResponsibilityToAuraRestore, parseArgs };
