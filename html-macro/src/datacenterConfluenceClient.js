...
class DatacenterConfluenceClient {
   ...
   *
   * @param {object} [opts]
   * @param {("global"|"personal"|null)} [opts.type] - Filter by DC space type.
   *   "global"  → site spaces only (recommended when targeting Cloud, which has
   *               no personal spaces).
   *   "personal" → personal spaces only (keys prefixed with "~").
   *   null/undefined → all spaces (no filter).
   * @returns {Array<{key: string, name: string, type: string}>}
   */
  async getAllSpaces(opts = {}) {
    ...
    while (true) {
      const response = await this.makeRequest(
        "GET",
        `/rest/api/space?limit=${limit}&start=${startAt}${typeQuery}`,
      );
      ...
      for (const space of results) {
        spaces.push({ key: space.key, name: space.name, type: space.type });
      }
